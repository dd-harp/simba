...
*This software was designed with features to improve simulation-based analytics while lowering costs, including time spent by humans building models, computational requirements, and human time required post-processing to interpret the output of the models.*

------------------------------------------------------------------------

This website supports development and dissemination of several GitHub R packages to support **Sim**ulation-**B**ased **A**nalytics (**SimBA**) for malaria and other mosquito-transmitted pathogens. Core computation is handled in [**`ramp.xds`**](ramp.xds.html), which is supplemented by five satellite packages. There are also two supplementary stand-alone packages [(see R packages)](Rpackages.html).

The software implements a rigorous mathematical framework for building models that is modular, flexible, and extensible [@WuSL2023SpatialDynamics]. It was designed to lower the costs of building and analyzing models for malaria research and analytics. The guiding design principles were [*nimble model building*](nimble.qmd) and [*scalable complexity.*](scalable_complexity.qmd)
