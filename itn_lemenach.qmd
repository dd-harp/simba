...
We load `ramp.xds` and the satellite library that implements control, `ramp.forcing`.
...
```{r}
cover_par <- list(trend_par = makepar_F_sharkfin(D=90))
```
...
```{r}
tm = seq(0, 5*365, by = 5)
show_bednet_coverage(tm, itn_mod)
```
...
```{r}
xds_plot_EIR(model)
xds_plot_EIR(itn_mod, clrs = "red", add=TRUE)
```
...
The model developed by Le Menach *et al.* has been encoded in `ramp.forcing` with three functions:
