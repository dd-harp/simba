...
```{r}
library(ramp.xds)
macmod <- xds_setup(HPop=1, Loptions = list(Lambda=0.1))
...
```{r}
macmod <- change_XH_inits(macmod, 1, list(X=.01))
M <- get_MY_inits(macmod)$M
P <- get_MY_inits(macmod)$P
macmod <- change_MY_inits(macmod, 1, list(M=M, P=P, Y=.01, Z=0))
