...
***Basic setup** can configure some basic human population strata, but it leaves setup for human demography, stratification by age, and other issues requiring stratification to [**advanced setup.**](advanced_setup.qmd) (see [Stratification](strafication.qmd))*
...
-   `xds_setup_cohort()` -- this model sets up the **XH** component, but it lacks an **MY** component and an **L** component. Instead, it configures a function $F_E(a,t, \tau)$ that computes the daily EIR for a cohort of humans born on day $\tau$ as it ages ($a$) over time ($t$). For more, see [Cohort Dynamics](cohort_dynamics.qmd).
