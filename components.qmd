...
The **MY** Component is specified by setting `MYname =` *`model_name`*, where *`model_name`* is a string that defines the `class` for S3 methods, or equivalently, a model family in **`ramp.xds`** or **`ramp.library`**
