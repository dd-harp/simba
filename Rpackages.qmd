...
+ **Simulation-Based Analytics** -- Six packages implement simulation-based analytics: core computation in [`ramp.xds`](ramp.xds.html) is supported by five satellite packages. Altogether, these R packages implement a modular framework for model building describe in *Spatial Dynamics of Malaria Transmission* [@WuSL2023SpatialDynamics]. We have also included two packages to explore concepts at the cutting edge of malaria epidemiology and microsimulation. Meanwhile, some older software packages -- predecessors to **`ramp.xds`** -- have been [deprecated](deprecated.html).

## Robust Analytics

Six packages support robust analytics for malaria policy. 
