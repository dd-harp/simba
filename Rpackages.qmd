...
+ **Simulation-Based Analytics** -- Seven packages implement simulation-based analytics: core computation in [`ramp.xds`](ramp.xds.html) is supported by six satellite packages. Altogether, these R packages implement a modular framework for model building describe in *Spatial Dynamics of Malaria Transmission* [@WuSL2023SpatialDynamics]. 

+ We have also included two packages to explore concepts at the cutting edge of malaria epidemiology and microsimulation. Meanwhile, some older software packages -- predecessors to **`ramp.xds`** -- have been [deprecated](deprecated.html).

+ One package **`ramp.func`** is used by all the other packages. 

***

![](Figures/ramp_packages.drawio.html){width=749px height=456px}

***

## SimBA

SimBA -- defined narrowly -- includes seven packages that support robust analytics for malaria policy. 
...
-   [**`ramp.func`**](ramp.func.html) is a supplementary code library for **`ramp.xds`** that holds algorithms to implement various models of malaria control, including vector control, mass vaccination, mass drug administration, *etc.*
...
-   [**`ramp.qa`**](ramp.qa.html) is a set of algorithms and functions to for qualitative analysis, including thresholds, connectivity, seasonality, and scaling relationships. 

-   [**`ramp.work`**](ramp.work.html) is a set of algorithms and functions to fit models to data, and to apply malaria models in context.
