...
```{r}
sin_mod$L_obj[[1]]$season_par$phase
```
...
```{r}
Lo = list(season_par <- makepar_F_sin()) 
sin_mod1 <- xds_setup(Loptions = Lo)
sin_mod1$L_obj[[1]]$season_par
```
...
trend_mod <- xds_setup(Loptions = Lo)
```
