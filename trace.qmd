...
+ $E(t)$ or `eir` --- the daily entomological inoculation rate (EIR) for a cohort of humans (or the force of infection in a population or a cohort, FoI) is set up by `xds_setup_eir`. The EIR is passed to the **XH** module by a trace function from the EIR object, which is functionally equivalent to an **MY** module. 

The first four terms are passed from trivial modules of the dynamical components. Examples are are documented in their vignettes. Since forcing by the **EIR** is handled slightly differently, it is the focus of @sec-eir: **EIR forcing** [(below)](#sec-eir), which presents some code to illustrate how setup for trace functions works. 
...
+ A library of shock functions is maintained as part of [`ramp.func`](https://dd-harp.github.io/ramp.func/articles/Shocks.html)
